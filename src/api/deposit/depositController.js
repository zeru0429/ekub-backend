...
const depositController = {
  ...
  update: async (req, res, next) => {
    ...
    try {
      ...
      if (cumulativePayment > category.totalAmount) {
        return res.status(400).json({
          success: false,
          message: `Deposit amount exceeds the remaining amount. You can only deposit up to ${
            parseFloat(lot.remainingAmount) +
            parseFloat(existingDeposit.amount) +
            parseFloat(existingDeposit.commition)
          }.`,
        });
      }
  ...
  getDepositsGroupedByDay: async (req, res, next) => {
    try {
      ...
      const firstDepositDate = startOfDay(
        new Date(Math.min(...deposits.map((d) => new Date(d.createdAt))))
      );
      const lastDepositDate = startOfDay(
        new Date(Math.max(...deposits.map((d) => new Date(d.createdAt))))
      );
  ...
  getMonthlyDepositsForYear: async (req, res, next) => {
    try {
      ...
      // Format the response
      const monthlyDepositsFormatted = monthlyDeposits.map(
        (deposits, index) => ({
    ...
    try {
      ...
      if (isNaN(week) || week < 1 || week > 52) {
        return res.status(400).json({
          success: false,
          message:
            "Invalid week parameter. Please provide a valid week (1-52).",
        });
      }

      // Calculate the start and end dates for the specified week of the year
      const startOfYearDate = new Date(year, 0, 1);
      const startOfWeekDate = startOfWeek(addWeeks(startOfYearDate, week - 1), {
        weekStartsOn: 1,
      });
  ...
  getDepositsForPeriodWithDeposit: async (req, res, next) => {
    try {
      ...
      if (weekNumber === 1) {
        ...
        startDate = addDays(
          startOfWeek(startOfMonthDate, { weekStartsOn: 1 }),
          7
        );
        endDate = addDays(startDate, 6);
  ...
  getRemainingDepositsForPeriod: async (req, res, next) => {
    try {
      ...
      if (
        weekNumber !== null &&
        (isNaN(weekNumber) || weekNumber < 1 || weekNumber > 52)
      ) {
        return res.status(400).json({
          success: false,
          message: "Invalid week number specified",
      ...
      if (month === null && weekNumber === null) {
        ...
        startDate = startOfWeek(startOfYear(new Date(year, 0, 1)), {
          weekStartsOn: 1,
        });
        ...
        startDate = addWeeks(
          startOfWeek(firstDayOfYear, { weekStartsOn: 1 }),
          weekNumber - 1
        );
        endDate = endOfWeek(startDate, { weekStartsOn: 1 });
      ...
      const filteredAndSortedDeposits = deposits.sort(
        (a, b) => parseFloat(b.remaining) - parseFloat(a.remaining)
      );
